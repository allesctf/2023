...
php wp-cli.phar --allow-root core  download --version=5.9.3 
php wp-cli.phar --allow-root config create --dbname=$WORDPRESS_DB_NAME --dbuser=$WORDPRESS_DB_USER --dbpass=$WORDPRESS_DB_PASSWORD --dbhost=$WORDPRESS_DB_HOST --skip-check
...
echo "127.0.0.1 $SESSIONID-1024-diff-the-patch.$CHALLENGE_DOMAIN" >> /etc/hosts
