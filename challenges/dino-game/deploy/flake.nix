  ...
  outputs = { self, nixpkgs, poetry2nix, nixgl_ }:
    let
      ...
      overrides = pkgs: pkgs.poetry2nix.overrides.withDefaults (pyself: pysuper: {
        ...
        native = pysuper.native.overridePythonAttrs (oldAttrs: {
          propagatedBuildInputs = (oldAttrs.propagatedBuildInputs or [ ]) ++ [ pkgs.stdenv.cc.cc.lib ];
        });
      });
    in
    {
      ...
      devShells = forAllSystems (system: {
        default = pkgs.${system}.mkShell {
          ...
          shellHook = ''
            ...
            export PYTHONPATH="$(pwd)/src/client:$(pwd)/src/server:$(pwd)/src/shared:$PYTHONPATH:$(pwd)/src/headless:$PYTHONPATH"
          '';
        };
      });
    };
